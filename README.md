...
| `npm run lint` | oxlint |
| `npm run build:docs` | Régénère le [site de documentation](#-documentation-utilisateur) (`dist/docs/`) sans reconstruire l'app — utile en itérant sur `docs-site/` |
...
**Tout est traduit** : les écrans, les mentions légales (`legal-notice.ts`), le guide de découpe (`decoupe-guide.ts`), l'encyclopédie (`encyclopedia.ts`), les catégories d'accords, le tableau de schéma JSON de l'écran Import/Export, l'écran À propos (y compris l'e-mail de support généré) — et, côté contenu des fiches, les **216 fiches**, région par région : Auvergne-Rhône-Alpes (49), Bourgogne-Franche-Comté (17), Bretagne (11), Centre-Val de Loire (13), Normandie (13), Hauts-de-France (18), Corse (10), Grand Est (12), Île-de-France (11), Occitanie (14), Nouvelle-Aquitaine (19), Provence-Alpes-Côte d'Azur (20) et Pays de la Loire (9) — soit `histoire`, `fabrication`, `conservation`, `service`, `anecdote`, `notes`, `accords` sur chacune. Champ optionnel `Cheese.en?` (`cheese.types.ts`), lu par `CheeseDetailScreen` avec repli sur le français si absent ; recollage par une table `EXTRA_TRANSLATIONS` dans `cheeses-extra.ts`, fusionnée dans `dataset.ts` sur le même patron que `EXTRA_EDITORIAL`. `dataset.test.ts` (bloc « Traductions anglaises (en) ») porte le garde-fou de cohérence FR/EN, y compris un garde-fou global sur les 216 fiches. Le résumé Wikipédia (`wikipedia.extract`) reste volontairement en français dans les deux langues : c'est une citation sourcée, la traduire romprait le lien avec l'article cité. Le site de documentation (`/docs/`) n'est pas concerné.
...
FAQ, dépannage, mode hors connexion, changelog, limites connues et support ne
sont plus dans ce README : ils vivent dans un vrai site de documentation,
généré statiquement et servi sous `/docs/` sur le même déploiement Netlify —
conforme à l'architecture de `DOCUMENTATION_SPEC.md` (§3), avec une page par
écran, par fonctionnalité, navigation en accordéons et recherche.

| | |
| --- | --- |
| **Où** | [fromageor.netlify.app/docs/](https://fromageor.netlify.app/docs/), et menu latéral de l'app → tout en bas → « Documentation » |
| **Source** | `docs-site/` — un composant `.tsx` par page (`content/`), layout et nav communs (`components/`), le tout compilé statiquement |
| **Build** | `scripts/build-docs.mjs` compile `docs-site/ssr-entry.tsx` via l'API de Vite (mode SSR, réutilise `@vitejs/plugin-react`) puis appelle `renderToStaticMarkup` page par page — zéro framework expédié au visiteur, seule la recherche a un peu de JS (`docs-site/search-client.js`, vanilla) |
| **Quand** | Chaîné dans `npm run build` (après `vite build`) ; `npm run build:docs` seul pour itérer sans reconstruire l'app |
| **Garde-fou** | `docs-site/ssr-entry.tsx` fait échouer le build si une page du contenu n'a pas d'entrée dans `docs-site/lib/nav.ts`, ou l'inverse — pas de lien mort, pas de page orpheline (§37/§47 de la spec) |

⚠️ Le site de doc ne doit **jamais** entrer dans le précache PWA de l'application : `vite.config.ts` l'exclut explicitement (`globIgnores` et `navigateFallbackDenylist` du plugin PWA), en plus de son propre dossier de sortie (`dist/docs/`, écrit après `vite build`).

Les mentions légales restent **la seule exception** : `docs-site/content/legal.tsx` ne reproduit pas leur texte, et renvoie vers l'écran de l'application (`src/lib/legal-notice.ts`) pour n'avoir qu'une seule source validable humainement.
...
| Fichier | Rôle |
| --- | --- |
| `README.md` | Ce fichier — fonctionnement du projet, données, scripts, écrans, déploiement. Public : développeur |
| `docs-site/` | Le site de documentation utilisateur servi sous `/docs/` — voir [📚 Documentation utilisateur](#-documentation-utilisateur) |
...
| `DOCUMENTATION_SPEC.md` | Le standard documentaire que le projet applique |
| `CLAUDE.md` | Contexte projet pour Claude Code, qui renvoie aux quatre précédents |
